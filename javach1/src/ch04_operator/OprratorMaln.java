...
public class OprratorMaln {
	...
	public static void main(String[] args) {
			...
			int inputAge = 24;
			String inputAgePhone = "01099796408";
			
			//나이는 14세 이상, 휴대폰 번호는 11자리 체크
			
			System.out.println(inputAge >= 14);
			System.out.println(inputAgePhone.length() == 11);
			
			//&&
			//좌우 조건이 전부  true여야 true 리턴
			//좌우 둘 중 하나라도 false라면 false 리턴
			System.out.println(inputAge >= 14 && inputAgePhone.length() == 11);
			
			// 휴대폰 번호는 10자리 혹은 11자리 여야 한다.
			// 10<= x <= 11
			//System.out.println(10<= inputAgePhone.length() <= 11); 이렇게 하면 안됨.
			System.out.println(10 <= inputAgePhone.length() 
					               && inputAgePhone.length() <= 11);
			
			
			// ||
			// 좌우 조건 중 하나라도 true면 true 리턴
			// 둘다 false 일 때만 false 리턴
			
			// X == 10 or X ==1
			
			System.out.println(inputAgePhone.length() == 10 
					       || inputAgePhone.length() == 11);
			
			//회원가입 예시
			String name = "차무식";
			String phone = "01099796408";
			int age = 50;
			// name은 empty면 안되고
			// phone은 10자리 또는 11자리
			// age 14세 이상
