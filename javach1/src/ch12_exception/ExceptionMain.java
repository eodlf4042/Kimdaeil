...
public class ExceptionMain {
	...
	public static void main(String[] args) throws ParseException {
		...
		try {
			...
			// try 블록 내에서 발생할 수 있는 헤어 종류에 대한
			// 클래스. 해당 에러가 발생하면 catch 블록 내부 실행 
			System.out.println("예외 발생!!");
			System.out.println("예러 메세지: " + except.getMessage());
		...
		System.err.println("빨간글씨 차차");
			...
			try {
				...
				System.out.println("예러 메세지: " + e.getMessage());
				e.printStackTrace();
			}
...
//		
		...
		// 시스템 에러는 안남 -> 우리가 직접 에러를 발생시킨다.
		
		// 충분히 예측되는 예외의 경우
		// 따로 Exception 클래스를 만들어서 다룬다.
		// 비즈니스 Exception -> BizExcption
		...
		} catch (BizException e) { // Throw BizException 객체가 들어온다.
			e.printStackTrace();
		...
		System.out.println("이름 출력 끝!");
			...
			try {
				...
				// 예외가 발생하든 안하든 실행될 명령어 입력  --보통은 이렇게 안씀
				System.out.println("항상 실행");
				// 주로 try블록 내에서 사용된 객체를 close할때 사용
			}
